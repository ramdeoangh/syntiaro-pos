...
export var order_mixin = {
    methods: {
        ...
        update_prices(){
            ...
            var additional_discount_amount = 0.00;
            ...
            for ( var product_slack in this.cart) {
                ...
                if(typeof product_data.selected_addon_products != 'undefined' && product_data.selected_addon_products != null){
                    for(var addon_product in product_data.selected_addon_products){
                        ...
                        if(!isNaN(quantity)){
                            var total_price = parseFloat(quantity)*parseFloat(addon_product_data.price);
                            var total_discount = this.calculate_discount(total_price, addon_product_data.discount_percentage);
                            var total_after_discount = parseFloat(total_price)-parseFloat(total_discount);
                            var total_tax = this.calculate_tax(total_after_discount, addon_product_data.tax_percentage);

                            this.$set(this.cart[product_slack]['selected_addon_products'][addon_product], 'total_price', total_price.toFixed(2));
                            this.$set(this.cart[product_slack]['selected_addon_products'][addon_product], 'total_tax', total_tax);
                            this.$set(this.cart[product_slack]['selected_addon_products'][addon_product], 'total_discount', total_discount);
